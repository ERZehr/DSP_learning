...
function [outputSignal] = applyPolyphaseFilters(signal, polyphaseFilters, interpolationFactor)
    ...
    outputSignal = primeStrip(length(polyphaseFilters), outputSignal, interpolationFactor, length(polyphaseFilters)/interpolationFactor);
end
...
function [primedSignal] = primeSignal(signalEncodeDimensions, signal, interpolationFactor, span)
    primePaddingLen = (interpolationFactor*span) + mod((interpolationFactor*span), signalEncodeDimensions); % number of transient bits we can bleed off without touching the original encoding
    primePadding = mod(0:primePaddingLen-1, 2); % alternating 1 and 0 of length primePaddingLen
    primedSignal = [primePadding signal]; 
end

% strip transient primeing to signal
function [strippedSignal] = primeStrip(signalEncodeDimensions, signal, interpolationFactor, span)
    primePaddingLen = interpolationFactor * (interpolationFactor*span) + mod((interpolationFactor*span), signalEncodeDimensions);
