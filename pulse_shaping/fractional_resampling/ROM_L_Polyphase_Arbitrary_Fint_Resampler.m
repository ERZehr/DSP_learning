...
% Efficient_Resampling_Implementations_DSP_Tips_amp_Tricks.pdf IEEE
F_int = 12.6374;
C = 1;
phi_int = C/F_int;
NCO_accum = 0;
...
for k = 1:(length(x)+row_size-1) % ensure the whole signal runs through a zero initialized regester and runs all the way out
    ...
    for p = 1:column_size % arbitrary L counter
        fprintf('NCO_accum=%d\n', NCO_accum);
        if NCO_accum >= C
            fprintf('Rollover at k=%d\n', k);
            NCO_accum = NCO_accum - C;
            ROM_addr_p = 1;
            if k <= length(x)
                shift_register = [x(k); shift_register(1:end-1)];
            else
